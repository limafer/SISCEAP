...
def AplicarProva(discEsc,nq,top,tipo):
    ...
    if tipo == 'P':
        ...
        print(f"Número de questões da Listaaa:{nq}")
        std = discEsc.split('-')[0].rstrip()
        lista = []
...
def elaborarProva():
    ...
    with st.container(border=True):
        ...
        with col1:
            ...
            st.header(':blue[Elaborar prova por Tópico]')
            ...
            if st.button("Seleção aleatória",help=':green[Todas as questões serão selecionadas aleatoriamente!]'):
            ...
            if st.button('Seleção Mista',help=':green[O usuário escolhe algumas questões, e o restante é escolhido aleatoriamente!]'):
                st.session_state.chk_disable = False
