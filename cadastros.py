    ...
    with container:
        ...
        with col2:
            ...
            if select == 'Alunos':
                ...
                st.dataframe(alunos,hide_index=True,width=900)
...
def atualiza_toggle():
    if not st.session_state.status_chk:
        st.session_state.status_tog = False
        
    st.session_state.status_chk = True
    st.write('')
        

def compilaQuestao(questao):
    if questao:
        geraTexImagem(questao)
        st.session_state.texto_area = questao
        st.write('Questão visualizada!')
    ...
    with container:
        ...
        with col1:
            ...
            if cad == 'Incluir Questões':
                ...
                on = st.toggle('Compila questão',key='status_tog',on_change=compilaQuestao,args=(questao,))

                if on:
                    check = st.checkbox('Cadastra a questão?',key='status_chk',on_change=atualiza_toggle)
                    if check:
                        CadastraQuestao(codquest,questao)
                    else:
                        st.session_state.texto_area = ""
